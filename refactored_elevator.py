...
def second_floor(items):
    ...
    print_pause("After a few moments, you find yourself in the human resources "
                "department.")
    ...
    if "handbook" in items:
        ...
        if "ID card" in items:
            ...
            print_pause("He has something for you,"
                        "but says he can't give it to you"
...
def third_floor(items):
    ...
    if "ID card" in items:
        ...
        print_pause("Your program manager greets you and"
                    "tells you that you need to have a copy of the"
                    "employee handbook in order to start work.")

        if "handbook" in items:
            print_pause("Fortunately, you got that from HR!")
            print_pause("Congratulations! You are ready to start your new job as "
